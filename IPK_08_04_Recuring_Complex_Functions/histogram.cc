...
void Histogram::cutDataSet(unsigned int n){
	std::vector<int> percentage_data= initialize(1000);
	double current=0;
	double min=0;
	double max=0;
	for(auto entry:percentage_data){
		current+=entry;
		if((current/data_set.size())*100>100-n){
			max=current;
			break;
		}
		if((current/data_set.size())*100>n &&min==0) min=current;
	}
	data_set.erase(data_set.begin(),data_set.begin()+min);
	data_set.erase(data_set.begin()+max-min,data_set.end());
}
